...
class LegalDocumentChunker:
    ...
    def chunk_legal_document(self, text: str, document_id: str, rule_title: str) -> list[dict]:
        """
        ...
        if token_count <= self.max_tokens:
            return [
                {"text": text, "token_count": token_count, "chunk_index": 0, "total_chunks": 1, "needs_chunking": False}
            ]
        ...
        for i, chunk_data in enumerate(bounded_chunks):
            formatted_text = self.create_chunk_with_context(
                ...
                total_chunks,
                rule_title,
                chunk_data.get("section_context", ""),
            )
    ...
    def _find_safe_break_point(self, text: str, start: int, end: int, section_context: str) -> int:
    ...
    def _split_large_text(self, text: str, section_context: str) -> list[dict]:
        ...
        while current_pos < len(text):
            ...
            break_point = self._find_safe_break_point(text, current_pos, estimated_end, section_context)
    ...
    def _fallback_sentence_chunking(self, text: str, document_id: str, rule_title: str) -> list[dict]:
        ...
        for i, chunk_text in enumerate(chunks):
            formatted_text = self.create_chunk_with_context(chunk_text, 0, len(chunk_text), i, total_chunks, rule_title)
