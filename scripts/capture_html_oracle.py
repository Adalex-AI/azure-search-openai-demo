...
from html_schema_oracle import (
    ORACLE_VERSION,
...
def capture_source(
    ...
    for attempt in range(attempts):
        ...
        raise RuntimeError("HTML capture exhausted without a result")
...
def run(
    ...
    summary = {
        ...
        "unavailable_results": [result for result in results if result.get("status") == "unavailable"],
        "results": results,
    }
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--retry-delay", type=float, default=1.0, help="Initial retry backoff in seconds")
