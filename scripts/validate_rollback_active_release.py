...
def _expected(plan: dict[str, Any]) -> dict[str, str]:
    fields = {key: str(plan.get(key) or "").strip() for key in ("image_digest", "revision_name", "release_id", "search_index", "knowledge_base", "platform")}
...
def _environment(app: Any, platform: str) -> dict[str, str]:
    if platform == "appservice":
        ...
        return {str(item.get("name")): str(item.get("value") or "") for item in app if isinstance(item, dict) and item.get("name")}
    ...
    values = {str(item.get("name")): str(item.get("value") or "") for item in environment if isinstance(item, dict) and item.get("name")}
    ...
    if not isinstance(traffic, list) or sum(int(item.get("weight") or 0) for item in traffic if isinstance(item, dict) and item.get("revisionName") == revision) != 100:
...
def validate_active_rollback(app: Any, plan: dict[str, Any], platform: str) -> str:
    ...
    for field in ("image_digest", "revision_name", "release_id", "search_index", "knowledge_base"):
        actual_key = {"image_digest": "V4_IMAGE_DIGEST", "revision_name": "V4_REVISION_NAME", "release_id": "V4_RELEASE_ID", "search_index": "AZURE_SEARCH_INDEX", "knowledge_base": "AZURE_SEARCH_KNOWLEDGEBASE_NAME"}[field]
