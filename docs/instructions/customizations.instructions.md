...
2. Use feature flags from `config.py` (backend) or `config.ts` (frontend)
3. Export all public APIs through `__init__.py` (backend) or `index.ts` (frontend)
4. Add `# CUSTOM:` comments when integrating into upstream files

### Backend Customizations
1. Place new routes in `customizations/routes/` as Flask Blueprints
2. Place approach helpers in `customizations/approaches/`
3. Use `is_feature_enabled("feature_name")` for conditional features
4. Follow existing patterns in `citation_builder.py` and `source_processor.py`

### Frontend Customizations
1. Export all utilities through `customizations/index.ts`
2. Use `isFeatureEnabled("featureName")` for conditional features
3. Place tests in `customizations/__tests__/`
...
2. Never use comma-separated citations like `[1, 2, 3]` or `1, 2, 3`
3. Each sentence should end with exactly one citation
4. The `sanitizeCitations()` function fixes malformed citations
