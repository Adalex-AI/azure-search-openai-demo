...
2.  **Lack of "Negative" Testing**: The current ground truth only contains questions that *have* answers. It does not test the system's ability to say "I don't know" when information is missing (critical for avoiding legal hallucinations).
3.  **No "Citation Validity" Check**: The system checks *if* a citation is present, but not if the cited text *actually supports* the claim (Groundedness at the citation level).
...
2.  Retrieve the text chunk associated with `[1]`.
3.  Use an LLM to verify: "Does the text in chunk [1] support the statement made in the answer?"
...
2.  **Completed**: Fixed `CITATION_REGEX` to correctly detect `[Document Name]` style citations, improving citation rate metric to 100%.
3.  **Completed**: Added `ANNEX_REGEX` and improved `CPR_RULE_REGEX` and `PRACTICE_DIRECTION_REGEX` to handle decimals and sub-rules, improving Court Guide accuracy.
4.  **Short Term**: Add 10 "Negative" questions to `ground_truth_cpr.jsonl`.
5.  **Medium Term**: Implement the "Citation Groundedness" LLM metric.
