...
2. **Mixed bracketed and unbracketed numbers**: `1, 2, [3]` or `123[10.15]`
3. **Complex nested citations**: `123[10.15–10.17, 10.25–10.27]`
4. **Commas inside brackets**: `[1, 2, 3]`
5. **Commas between bracketed citations**: `[1], [2], [3]`
...
2. `/app/backend/approaches/prompts/ask_answer_question.prompty`
...
2. **Clear examples of correct formats**:
   ```
   ✓ [1]
   ✓ [1][2]
   ✓ [1][2][3]
...
3. **Clear examples of incorrect formats (explicitly prohibited)**:
   ```
   ✗ 1, 2, 3
   ✗ [1, 2, 3]
   ✗ 1, 2, [3]
...
2. The prompt files have been updated
3. Restart the application using `./app/start.sh` or the "Start App" task
4. Test with sample queries to verify citation format
5. Monitor for any edge cases where citations might still be malformed

## Rollback Instructions

If the changes cause issues, revert by:
1. Remove the **CRITICAL CITATION FORMAT RULES** section from both prompty files
2. Keep only the original **CRITICAL INSTRUCTIONS** section
