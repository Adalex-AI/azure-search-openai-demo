...
def test_production_evidence_rebuild_reconstructs_cross_job_provenance():
    ...
    assert workflow.index('AZURE_SEARCH_SERVICE: ${{ secrets.AZURE_SEARCH_SERVICE }}', workflow.index('name: Rebuild evidence with Production approval')) > 0
...
def test_workflow_serializes_production_actions_and_rejects_conflicting_inputs():
    ...
    assert "if: inputs.rollback != true && (inputs.promote == true || (github.event_name == 'schedule' && vars.V4_AUTO_PROMOTE == 'true'))" in workflow
